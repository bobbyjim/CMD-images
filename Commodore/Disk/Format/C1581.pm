package Commodore::Disk::Format::C1581;
...
Commodore::Disk::Format::C1581 - D81/D65 format file for the Commodore disk image reader.
...
This file contains parametric data needed for correctly reading to and writing from a D81 or D65 file.
...
=cut
   ...
   sub format          { 'D81' }
   sub DOStype         { '3D' }                           # 1 byte
   sub HDR_DIR_track   { 0x28 }                           # 1 byte
   sub HDRlabelOffset  { 0x04 }                           # 1 byte
   sub DIRinterleave   { 1 }                              # 1 byte
   sub FILinterleave   { 1 }                              # 1 byte
   sub BAMlabelOffset  { 0x10 }                           # 1 byte

   sub zones
   {
      (
         { 'highTrack' => 20, 'sectorsPerTrack' => 40 },  
         { 'highTrack' => 40, 'sectorsPerTrack' => 40 },
         { 'highTrack' => 60, 'sectorsPerTrack' => 40 },  
         { 'highTrack' => 80, 'sectorsPerTrack' => 40 },  # 8 bytes
      );
   };

   sub BAMinterleave   { 1 }                              # 1 byte
   sub BAMlocation     { $BAM_FOLLOWS_HDR }               # 1 byte
   sub BootTrack       { 0x00 }                           # 1 byte

   sub BAMsectors        { 2 }  
   sub tracksPerBAMsector{ 40 } 
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc Commodore::Disk::Format::C1581
...
1; # End of Commodore::Disk::Format::C1581
