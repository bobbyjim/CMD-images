package Commodore::Disk::Format::C2040;
...
Commodore::Disk::Format::C2040 - D67/D40 format file for the Commodore disk image reader.
...
This file contains parametric data needed for correctly reading to and writing from a D67 or D40 file.
...
This drive has DOS version 1. DOS1 uses a disk type closely related 
to the 1541 disk image. The primary difference is that tracks 18-24 on DOS1 disks 
have 20 sectors rather than the 19 found on 1541 disks. 
DOS1 disk images have the extension .D67. 
...
=cut
   ...
   sub format          { 'D67' }       # also 'D40'...
   sub DOStype         { '1A' }                           # 1 byte
   sub HDR_DIR_track   { 0x12 }                           # 1 byte
   sub HDRlabelOffset  { 0x90 }                           # 1 byte
   sub DIRinterleave   { 3 }                              # 1 byte
   ...
   sub zones
   {
      (
         ...
         { 'highTrack' => 24, 'sectorsPerTrack' => 20 }, #### <======= ####
         { 'highTrack' => 30, 'sectorsPerTrack' => 18 },  
         { 'highTrack' => 35, 'sectorsPerTrack' => 17 },  # 8 bytes
      );
   };

   sub BAMinterleave   { 0 }                              # 1 byte
   sub BAMlocation     { $BAM_ON_HDR }                    # 1 byte
   sub BootTrack       { 0x00 }                           # 1 byte

   sub BAMsectors        { 1 }  # ?? Math.ceil( (TRACKS * BAM_BYTES_PER_TRACK)/(254-BAMlabelOffset) );
   sub tracksPerBAMsector{ 35 } # ?? Math.floor(TRACKS/bamSectors);
...
1; # End of Commodore::Disk::Format::C2040
