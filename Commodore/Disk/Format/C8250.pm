package Commodore::Disk::Format::C8250;
...
Commodore::Disk::Format::C8250 - D82 format file for the Commodore disk image reader.
...
This file contains parametric data needed for correctly reading to and writing from a D80 file.
...
=cut
   ...
   sub FILinterleave   { 1 }                              # 1 byte
   sub BAMlabelOffset  { 0x06 }                           # 1 byte
   ...
   sub zones
   {
      (
         ...
         { 'highTrack' => 77+39, 'sectorsPerTrack' => 29 }, 
         { 'highTrack' => 77+53, 'sectorsPerTrack' => 27 },
         { 'highTrack' => 77+64, 'sectorsPerTrack' => 25 },  
         { 'highTrack' => 77+77, 'sectorsPerTrack' => 23 },		 
      );
   };

   sub BAMinterleave   { 3 }                              # 1 byte
   sub BAMlocation     { $BAM_TRACK_BEFORE_HDR }          # 1 byte
   sub BootTrack       { 0x00 }                           # 1 byte

   sub BAMsectors        { 4 }  # ?? Math.ceil( (TRACKS * BAM_BYTES_PER_TRACK)/(254-BAMlabelOffset) );
   sub tracksPerBAMsector{ 50 } # ?? Math.floor(TRACKS/bamSectors);
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc Commodore::Disk::Format::C8250
...
1; # End of Commodore::Disk::Format::C8250
