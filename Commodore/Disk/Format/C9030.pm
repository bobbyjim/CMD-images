package Commodore::Disk::Format::C9030;
...
Commodore::Disk::Format::C9030 - D93 format file for the Commodore disk image reader.
...
our $VERSION = '1.00';
   ...
   my $BAM_SPILLS_OVER        = 0x47; # = decimal 71; i.e. "1571 mode"
   my $BAM_STEALS_FROM_ZONES  = 0x5A; # = decimal 90; i.e. "90x0 mode"

=head1 SYNOPSIS

This file contains parametric data needed for correctly reading to and writing from a D93 file.

D93 - Commodore emulator file of a CBM-9030 diskette 

The 9030 is a hypothetical IEEE-488 hard drive with 2.5MB storage and Commodore DOS 3.0.

Unusual features of the 90x0 family include:
...
=cut
   ...
   {
      (
         { 'highTrack' =>  75, 'sectorsPerTrack' => 32 },  
         { 'highTrack' => 153, 'sectorsPerTrack' => 32 }, 
         { 'highTrack' => 229, 'sectorsPerTrack' => 32 },  
         { 'highTrack' => 306, 'sectorsPerTrack' => 32 },  # 8 bytes (really!)
      );
   };
   ...
   sub BAMsectors        {  7 } # ?? Math.ceil( (TRACKS * BAM_BYTES_PER_TRACK)/(254-BAMlabelOffset) );
